...
服务端首次启动时会根据机器名生成并保存稳定的本地 mDNS 域名，域名会转换为小写 ASCII 并移除空格，例如 `livingroompc.local`。之后即使机器名或服务端显示名变化，也会继续使用首次保存的域名。如需改名，可在服务端设置中修改 H5 `.local` 域名，或通过 `--local-domain` 指定。如果本地域名注册不可用，二维码会回退到局域网 IP 地址。
